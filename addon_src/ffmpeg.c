 ...
 */
 ...
 // 全局日志回调上下文
 static LogCallbackContext *g_log_context = NULL;
 
 // 自定义日志回调函数，实时调用 JavaScript 回调
 static void log_callback(void *ptr, int level, const char *fmt, va_list vl)
 {
     if (!g_log_context || !g_log_context->tsfn) {
         // 如果没有设置回调，使用默认输出
     ...
     char line[1024];
     int ret = vsnprintf(line, sizeof(line), fmt, vl);
     if (ret > 0 && ret < (int)sizeof(line)) {
         // 分配内存存储日志行（会在 call_js_callback 中释放）
         char *log_line = (char *)av_malloc(ret + 1);
         if (log_line) {
             strncpy(log_line, line, ret);
             log_line[ret] = '\0';
             // 调用线程安全的 JavaScript 回调
             napi_call_threadsafe_function(g_log_context->tsfn, log_line, napi_tsfn_nonblocking);
         }
     }
 }
 ...
 {
     if (env == NULL || js_callback == NULL || data == NULL) {
         return;
     }
     
     napi_status status;
     napi_value undefined, message;
     char *log_line = (char *)data;
     ...
     status = napi_create_string_utf8(env, log_line, NAPI_AUTO_LENGTH, &message);
     if (status != napi_ok) {
         av_free(log_line);
         return;
     }
     
     status = napi_call_function(env, undefined, js_callback, 1, &message, NULL);
  ...
  * 
  * Arguments:
  *   [0] - Array of FFmpeg command-line arguments
  *   [1] - Optional callback function for log output (line: string) => void
  */
 napi_value ffmpeg_run(napi_env env, napi_callback_info info)
 {
     ...
     LogCallbackContext log_context = {0};
     int log_callback_set = 0;
     ...
     if (argc >= 2) {
         ...
         status = napi_typeof(env, argv[1], &callback_type);
         if (status == napi_ok && callback_type == napi_function) {
             js_callback = argv[1];
             
             // 创建线程安全的函数引用
             napi_value async_resource_name;
             status = napi_create_string_utf8(env, "FFmpegLogCallback", NAPI_AUTO_LENGTH, &async_resource_name);
             if (status == napi_ok) {
                 status = napi_create_threadsafe_function(
                     ...
                     NULL,
                     async_resource_name,
                     0,  // max_queue_size: 0 means unlimited, but we'll manage it
                 ...
                 if (status == napi_ok && tsfn != NULL) {
                     // 不需要 acquire，创建时已经有一个引用
                     log_context.env = env;
                     log_context.tsfn = tsfn;
                     g_log_context = &log_context;
